...
import { Pokemon, TYPE_ICONS } from '@/constants/pokemon';
...
export function PokemonCard({ pokemon, size = 'large' }: PokemonCardProps) {
  const color = getColor(pokemon.types);
      ...
      <View style={[styles.innerCard, { backgroundColor: color.bg }]}>
        ...
        <View style={[styles.topBar, { backgroundColor: color.accent + '22', borderBottomColor: color.accent + '55' }]}>
          ...
          {!isMini && (
            <View style={styles.hpRow}>
              ...
              <Text style={[styles.hpValue, { color: color.accent }]}>{pokemon.hp}</Text>
            </View>
          )}
        </View>
        ...
        {isMini ? (
          <View style={[styles.footerMini, { borderTopColor: color.accent + '33' }]}>
            <Text style={[styles.numberMini, { color: color.accent + 'BB' }]}>#{pokemon.id}</Text>
            <View style={[styles.typePillMini, { backgroundColor: color.accent + '30' }]}>
              <Text style={[styles.typeEmojiMini]}>{TYPE_ICONS[pokemon.types[0]] ?? '⭐'}</Text>
            </View>
          </View>
        ) : (
          <View style={[styles.footerLarge, { borderTopColor: color.accent + '33' }]}>
            ...
            <Text style={[styles.numberLarge, { color: color.accent + 'BB' }]}>#{pokemon.id}</Text>
          </View>
        )}
        ...
        {!isMini && (
          <View style={[styles.statsRow, { borderTopColor: color.accent + '22' }]}>
            <View style={styles.statItem}>
              ...
              <Text style={[styles.statValue, { color: color.accent }]}>{pokemon.height}</Text>
            </View>
            <View style={[styles.statDivider, { backgroundColor: color.accent + '33' }]} />
            <View style={styles.statItem}>
              <Text style={styles.statLabel}>PESO</Text>
              <Text style={[styles.statValue, { color: color.accent }]}>{pokemon.weight}</Text>
            </View>
            <View style={[styles.statDivider, { backgroundColor: color.accent + '33' }]} />
            <View style={styles.statItem}>
              <Text style={styles.statLabel}>HABILIDADE</Text>
              <Text style={[styles.statValue, { color: color.accent }]} numberOfLines={1}>{pokemon.ability}</Text>
            </View>
          </View>
        )}
      </View>
